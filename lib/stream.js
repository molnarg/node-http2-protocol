...
Stream.prototype._initializeDataFlow = function _initializeDataFlow() {
  ...
  this.upstream.on('finish', this._onUpstreamFinish.bind(this));
...
// If the other end signals that it will never send a frame again (e.g. half-closing TCP):
// * If we expected more frames then emit error (in practice, this almost always means
//   that the other end won't receive our frames so emit error except in CLOSED state)
// * Signal that there will be no more frames
Stream.prototype._onUpstreamFinish = function _onUpstreamFinish() {
...
Stream.prototype._transition = function transition(sending, frame) {
  ...
  // Receiving and RST_STREAM that closes the stream
  if (receiving && RST_STREAM && (this.state == 'CLOSED') && (previousState !== 'CLOSED')) {
