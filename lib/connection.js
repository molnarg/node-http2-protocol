...
//
// * **Event: 'goaway' ([type])**: signals the receipt of a GOAWAY frame with an optional error code
//
// * **Event: 'stream' (stream)**: signals that there's an incoming stream
//
// * **createStream(): stream**: initiate a new stream (or return null if it's not possible)
//
...
//
// * **goaway([error])**: forbids creation of new stream for the remote endpoint and possibly
//   indicates an error
//
// * **close([error])**: send GOAWAY, and then close the underlying stream when there's no more
//   active stream
...
var MAX_STREAM_ID = require('./framer').MAX_STREAM_ID;
...
Connection.prototype._initializeStreamManagement = function _initializeStreamManagement(firstStreamId) {
  ...
  this.on('stream_count_change', this._onStreamCountChange)
};
...
Connection.prototype._onStreamCountChange = function _onStreamCountChange(change) {
  if (change) {
...
Connection.prototype._allocateId = function _allocateId(stream, id) {
  // * Can't allocate a new ID if the remote end doesn't want new incoming streams, or we reached
  //   the maximum stream ID
  if ((id === undefined) && (this._goneawayRemote)) {
    return undefined
  }
  ...
  if (id === undefined) {
    ...
    if (this._nextStreamId > MAX_STREAM_ID) {
      this._log.trace('Reached maxmimum outbound stream ID. Going into closed_remote state.');
      this._goneawayRemote = true;
    }
  }
...
Connection.prototype.createStream = function createStream() {
  if (this._goneawayRemote) {
    this._log.trace('Trying to create outbound stream after the other end closed the connection');
    return null;
  }
    ...
    while (bucket.length > 0) {
      for (var index = 0; index < bucket.length; index++) {
        ...
        if (frame.stream === undefined) {
          ...
          if (frame.stream === undefined) {
            this._log.trace({ s: stream, frame: frame }, 'Unsuccessful ID allocation, ' +
                                                         'terminating stream with REFUSED_STREAM.');
            stream.write({
              type: 'RST_STREAM',
              flags: {},
              stream: undefined,
        ...
        this.emit('stream_count_change', frame.count_change);
...
Connection.prototype._receive = function _receive(frame, done) {
  this._log.trace({ frame: frame }, 'Incoming frame');

  // * ignoring incoming frames that would initiate new stream if the connection is being closed
  if (this._goneawayLocal && (frame.stream > this._lastIncomingStream)) {
    this._log.trace({ frame: frame }, 'Ignoring incoming frame for a new stream since the ' +
                                      'connection is being closed by us');
    return;
  }
  ...
  frame.count_change = this.emit.bind(this, 'stream_count_change');
...
Connection.prototype._initializeLifecycleManagement = function _initializeLifecycleManagement() {
  ...
  this._goneawayLocal = false;
  this._goneawayRemote = false;
  this._closing = false;
  this._closed = false;
  this._activeStreamCount = 0;
  this.on('stream_count_change', this._onStreamCountChange2);
};
...
// Sending GOAWAY, which instructs the other side not to initiate new stream anymore. This can
// be called multiple times if a new error comes up.
Connection.prototype.goaway = function goaway(error) {
  this._log.debug({ error: error }, 'Sending GOAWAY');
  this._goneawayLocal = true;
  this.push({
    type: 'GOAWAY',
    flags: {},
    stream: 0,
...
// Receiving GOAWAY forbids initiating new streams
Connection.prototype._receiveGoaway = function _receiveGoaway(frame) {
  this._log.debug({ error: frame.error }, 'Received GOAWAY');
  this._goneawayRemote = true;

  // * Frames that are were in flight when the remote endpoint sent the GOAWAY must be terminated.
  //   We simulate this by creating a syntetic RST_STREAM with REFUSED_STREAM error code that
  //   indicates that the stream was not processed by the other end at all.
  for (var id in this._streamIds) {
    if (id > frame.last_stream) {
      this.write({
        type: 'RST_STREAM',
        flags: {},
        stream: id,
  ...
  // * Emitting en event to inform the user about the GOAWAY frame
  this.emit('goaway', (frame.error === 'NO_ERROR') ? undefined : frame.error);
};

// If the other end closes the TCP connection, this means:
// * It will never send a frame again so we can call `end()` on every stream (the finish event is
//   called when everything is flushed to the streams so it is safe to call `end()`)
// * If there are streams that expected more frames, then they will handle this by emitting error
// * Generally, HTTP2 is unusable with a half closed underlying stream, so we close too
Connection.prototype._onFinish = function _onFinish() {
  for (var id in this._streamIds) {
    this._streamIds[id].end();
  }
  this.push(null);
};

// Sending a GOAWAY, and closing the connection when there's no more connection
Connection.prototype.close = function close(error) {
  if (!this._closing && !this._closed) {
    this.goaway(error);
    this._closing = true;
    this._onStreamCountChange2(0);
  }
};

// Counting the active streams
Connection.prototype._onStreamCountChange2 = function _onStreamCountChange2(change) {
  this._activeStreamCount += change;
  if (this._closing && (this._activeStreamCount === 0)) {
    this._closing = false;
    this._closed = true;
    this.push(null);
  }
