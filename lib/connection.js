...
Connection.prototype._receive = function _receive(frame, done) {
  ...
  // * or creates one if it's not in `this.streamIds`
  if (!stream) {
...
Connection.prototype.notifySocketClosed = function notifySocketClosed(error) {
  if (this._closed) {
    return;
  }

  // Send GOAWAY, even though it probably won't get there.
  this.close(error);

  // Close all streams
  this._streamIds.forEach(function(stream, id) {
    if (id === 0) {
      return;
    }
